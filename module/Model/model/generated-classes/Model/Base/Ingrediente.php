...
/**
 * Base class that represents a row from the 'ingrediente' table.
 *
 * 
 *
* @package    propel.generator.Model.Base
*/
abstract class Ingrediente implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\Map\\IngredienteTableMap';
    ...
    /**
     * Initializes internal state of Model\Base\Ingrediente object.
     */
    ...
    /**
     * Compares this with another <code>Ingrediente</code> instance.  If
     * <code>obj</code> is an instance of <code>Ingrediente</code>, delegates to
     * <code>equals(Ingrediente)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Ingrediente The current object, for fluid interface
     */
     ...
     * @return $this|\Model\Ingrediente The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[IngredienteTableMap::COL_ID] = true;
        }
    ...
    public function setNome($v)
    {
        ...
        if ($this->nome !== $v) {
            $this->nome = $v;
            $this->modifiedColumns[IngredienteTableMap::COL_NOME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : IngredienteTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : IngredienteTableMap::translateFieldName('Nome', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = IngredienteTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Model\\Ingrediente'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildIngredienteQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Ingrediente::setDeleted()
     * @see Ingrediente::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildIngredienteQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                IngredienteTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[IngredienteTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . IngredienteTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(IngredienteTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(IngredienteTableMap::COL_NOME)) {
            $modifiedColumns[':p' . $index++]  = 'nome';
        }

        $sql = sprintf(
            'INSERT INTO ingrediente (%s) VALUES (%s)',
    ...
    {
        $pos = IngredienteTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Ingrediente'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Ingrediente'][$this->hashCode()] = true;
        $keys = IngredienteTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getNome(),
        );
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\Ingrediente The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(IngredienteTableMap::DATABASE_NAME);

        if ($this->isColumnModified(IngredienteTableMap::COL_ID)) {
            $criteria->add(IngredienteTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(IngredienteTableMap::COL_NOME)) {
            $criteria->add(IngredienteTableMap::COL_NOME, $this->nome);
        }
    ...
    {
        $criteria = ChildIngredienteQuery::create();
        $criteria->add(IngredienteTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Model\Ingrediente (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Model\Ingrediente Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildIngrediente is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildIngrediente The current object (for fluent API support)
     */
    public function setIngredienteReceitas(Collection $ingredienteReceitas, ConnectionInterface $con = null)
    {
        ...
        foreach ($ingredienteReceitasToDelete as $ingredienteReceitaRemoved) {
            $ingredienteReceitaRemoved->setIngrediente(null);
        }
    ...
    protected function doAddIngredienteReceita(ChildIngredienteReceita $ingredienteReceita)
    {
        ...
        $ingredienteReceita->setIngrediente($this);
    }
    ...
    public function removeIngredienteReceita(ChildIngredienteReceita $ingredienteReceita)
    {
        if ($this->getIngredienteReceitas()->contains($ingredienteReceita)) {
            ...
            $ingredienteReceita->setIngrediente(null);
        }
     ...
     * Otherwise if this Ingrediente is new, it will return
     * an empty collection; or if this Ingrediente has previously
     * been saved, it will retrieve related IngredienteReceitas from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Ingrediente.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    public function setProdutoBases(Collection $produtoBases, ConnectionInterface $con = null)
    {
        ...
        foreach ($produtoBasesToDelete as $produtoBaseRemoved) {
            $produtoBaseRemoved->setIngrediente(null);
        }
    ...
    protected function doAddProdutoBase(ChildProdutoBase $produtoBase)
    {
        ...
        $produtoBase->setIngrediente($this);
    }
    ...
    public function removeProdutoBase(ChildProdutoBase $produtoBase)
    {
        if ($this->getProdutoBases()->contains($produtoBase)) {
            ...
            $produtoBase->setIngrediente(null);
        }
     ...
     * @return ObjectCollection|ChildProdutoBase[] List of ChildProdutoBase objects
     */
    public function getProdutoBasesJoinUnidade(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildProdutoBaseQuery::create(null, $criteria);
        $query->joinWith('Unidade', $joinBehavior);
    ...
    {
        return (string) $this->exportTo(IngredienteTableMap::DEFAULT_STRING_FORMAT);
    }
