...
abstract class Cliente implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\Map\\ClienteTableMap';
    ...
    /**
     * Initializes internal state of Model\Base\Cliente object.
     */
     ...
     * @return $this|\Model\Cliente The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ClienteTableMap::translateFieldName('Endereco', TableMap::TYPE_PHPNAME, $indexType)];
            $this->endereco = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ClienteTableMap::translateFieldName('Telefone', TableMap::TYPE_PHPNAME, $indexType)];
            $this->telefone = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ClienteTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = ClienteTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Model\\Cliente'), 0, $e);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\Cliente The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Model\Cliente (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Model\Cliente Clone of current object.
     * @throws PropelException
     */
    ...
    public function initPedidos($overrideExisting = true)
    {
        ...
        $this->collPedidos = new ObjectCollection();
        $this->collPedidos->setModel('\Model\Pedido');
    }
     ...
     * @throws PropelException
     */
    public function countPedidos(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collPedidosPartial && !$this->isNew();
        if (null === $this->collPedidos || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collPedidos) {
     ...
     */
    protected function doAddPedido(ChildPedido $pedido)
    {
