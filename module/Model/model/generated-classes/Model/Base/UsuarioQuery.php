 ...
 * @method     ChildUsuarioQuery orderByHabilitado($order = Criteria::ASC) Order by the habilitado column
 *
 ...
 * @method     ChildUsuarioQuery groupByHabilitado() Group by the habilitado column
 *
 ...
 * @method     ChildUsuario findOneByHabilitado(boolean $habilitado) Return the first ChildUsuario filtered by the habilitado column *
 ...
 * @method     ChildUsuario requireOneByHabilitado(boolean $habilitado) Return the first ChildUsuario filtered by the habilitado column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUsuario[]|ObjectCollection findByHabilitado(boolean $habilitado) Return ChildUsuario objects filtered by the habilitado column
...
abstract class UsuarioQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, nome, email, senha, data_cadastro, habilitado FROM usuario WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByHabilitado(true); // WHERE habilitado = true
     * $query->filterByHabilitado('yes'); // WHERE habilitado = true
     * </code>
     *
     * @param     boolean|string $habilitado The value to use as filter.
    ...
    public function filterByHabilitado($habilitado = null, $comparison = null)
    {
        if (is_string($habilitado)) {
            $habilitado = in_array(strtolower($habilitado), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }

        return $this->addUsingAlias(UsuarioTableMap::COL_HABILITADO, $habilitado, $comparison);
    }
