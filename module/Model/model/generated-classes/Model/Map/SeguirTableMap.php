...
/**
 * This class defines the structure of the 'seguir' table.
 *
 *
 *
 ...
 *
 */
class SeguirTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Model.Map.SeguirTableMap';
     ...
     */
    const TABLE_NAME = 'seguir';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\Model\\Seguir';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Model.Seguir';
     ...
     */
    const COL_ID_PROPOSICAO = 'seguir.id_proposicao';

    /**
     * the column name for the id_usuario field
     */
    const COL_ID_USUARIO = 'seguir.id_usuario';

    /**
     * the column name for the data_cadastro field
     */
    const COL_DATA_CADASTRO = 'seguir.data_cadastro';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(SeguirTableMap::COL_ID_PROPOSICAO, SeguirTableMap::COL_ID_USUARIO, SeguirTableMap::COL_DATA_CADASTRO, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(SeguirTableMap::COL_ID_PROPOSICAO => 0, SeguirTableMap::COL_ID_USUARIO => 1, SeguirTableMap::COL_DATA_CADASTRO => 2, ),
    ...
    public function initialize()
    {
        // attributes
        $this->setName('seguir');
        $this->setPhpName('Seguir');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Model\\Seguir');
  ...
  array (
    ...
    {
        return array(
            'aggregate_column_relation_aggregate_seguir' => array('foreign_table' => 'proposicao', 'update_method' => 'updateQteSeguidores', 'aggregate_name' => 'QteSeguidores', ),
        );
    } // getBehaviors()
     ...
     * and findPk*() calls.
     *
     * @param \Model\Seguir $obj A \Model\Seguir object.
     ...
     *
     * @param mixed $value A \Model\Seguir object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \Model\Seguir) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \Model\Seguir object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? SeguirTableMap::CLASS_DEFAULT : SeguirTableMap::OM_CLASS;
    }
     ...
     * @return array           (Seguir object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = SeguirTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = SeguirTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + SeguirTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = SeguirTableMap::OM_CLASS;
            /** @var Seguir $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            SeguirTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = SeguirTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = SeguirTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(SeguirTableMap::COL_ID_PROPOSICAO);
            $criteria->addSelectColumn(SeguirTableMap::COL_ID_USUARIO);
            $criteria->addSelectColumn(SeguirTableMap::COL_DATA_CADASTRO);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(SeguirTableMap::DATABASE_NAME)->getTable(SeguirTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(SeguirTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(SeguirTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new SeguirTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Seguir or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Seguir object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Model\Seguir) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(SeguirTableMap::COL_ID_PROPOSICAO, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(SeguirTableMap::COL_ID_USUARIO, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = SeguirQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            SeguirTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                SeguirTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return SeguirQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Seguir or Criteria object.
     *
     * @param mixed               $criteria Criteria or Seguir object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Seguir object
        }
